    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl mx-auto">
        <form action="{{ route('warga.update', $warga) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input id="nik" name="nik" type="text" value="{{ old('nik', $warga->nik) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                @error('nik')
            ...
            <div class="mb-4">
                ...
                <input id="nama" name="nama" type="text" value="{{ old('nama', $warga->nama) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                @error('nama')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
